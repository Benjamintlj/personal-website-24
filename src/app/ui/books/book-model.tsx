...
export type ShelfOrigin = {
    ...
    clearance: number; cameraHeight: number
...
export function BookFaces({ opening = false, inside, backCover = false, artwork }: { opening?: boolean; inside?: ReactNode; backCover?: boolean; artwork?: BookArtwork }) {
    return <>
        <span className={`${styles.back} ${opening ? styles.inside : ''} ${artwork && !opening ? styles.darkSurface : ''}`} aria-hidden="true">
        ...
        {!opening && <>
            ...
            <span className={`${styles.spine} ${backCover ? styles.reverseSpine : ''} ${artwork ? styles.darkSurface : ''}`}>{artwork?.spine ?? <Image src={`${BOOK.assetRoot}/spine.webp`} alt="" width={94} height={1989} unoptimized />}</span>
        </>}
        <span className={`${styles.front} ${backCover ? styles.reverseFront : ''} ${artwork ? styles.darkSurface : ''}`}>{artwork ? (backCover ? artwork.back : artwork.front) : <Image src={`${BOOK.assetRoot}/${backCover ? 'back' : 'front'}-cover.webp`} alt={backCover ? 'Back cover' : 'Final Year Project, NTU — Benjamin Lewis-Jones'} width={1440} height={1989} unoptimized />}</span>
    </>
}
