...
![Untitled](Networking%20Topologies/Untitled.png)

---

## Bus

All hosts are connected via a transmission medium in the bus topology. Every host has access to the transmission medium and the signals that are transmitted over it. There is no central network 
component that controls the processes on it. The transmission medium for this can be, for example, a `coaxial cable`.
...
![Untitled](Networking%20Topologies/Untitled%201.png)
...
![Untitled](Networking%20Topologies/Untitled%202.png)
...
![Untitled](Networking%20Topologies/Untitled%203.png)

---

## Mesh

Many nodes decide about the connections on a `physical` level and the routing on a `logical`
level in meshed networks. Therefore, meshed structures have no fixed topology. There are two basic structures from the basic concept: the `fully meshed` and the `partially meshed` structure.
...
![Untitled](Networking%20Topologies/Untitled%204.png)
...
![Untitled](Networking%20Topologies/Untitled%205.png)

---

## Hybrid

Hybrid networks combine two or more topologies so that the resulting network does not present any standard topologies. For example, a tree network can represent a hybrid topology in which star networks are connected via interconnected bus networks. However, a tree network that 
is linked to another tree network is still topologically a tree network. A hybrid topology is always created when `two different` basic network topologies are interconnected.

### Hybrid Topology

![Untitled](Networking%20Topologies/Untitled%206.png)
...
![Untitled](Networking%20Topologies/Untitled%207.png)